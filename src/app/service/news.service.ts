...
export class NewsService {
  ...
  addPost(post: NewsInfoDto) {
    return this.http.post(`${environment.serverUrl}intsructions/addInstr`, post);
  }

  getNews() {
    return this.http.get<NewsInfoDto[]>(`${environment.serverUrl}instructions/getAll`);
  }

  getNewsByIdUser(idUser: number) {
    return this.http.get<NewsInfoDto[]>(`${environment.serverUrl}instructions/getedit` + idUser);
  }

  editPost(post: NewsInfoDto) {
    return this.http.post(`${environment.serverUrl}instructions/edit`, post);
  }
  ...
  searchByFragment(news: NewsInfoDto[], fragment: string): NewsInfoDto[] {
    ...
    news.forEach(function (post: NewsInfoDto) {
      if (post.name.toLowerCase().includes(fragment.toLowerCase())
        || post.description.toLowerCase().includes(fragment.toLowerCase())) {
        ans.push(post);
      }
    });
    return ans;
